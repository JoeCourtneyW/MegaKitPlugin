...
public class Squid extends Kit implements Listener{
	...
	public void SquidSplash(PlayerInteractEvent event){
		if(Kits.getClass(event.getPlayer()) == Kits.SQUID){
			if(Server.getInstance().isActivateAbilityEvent(event)){
				...
				WarpEntityEffect we = new WarpEntityEffect(Server.getEffectManager(), ef);
				we.radius = 2;
				...
				for(Entity e : p.getNearbyEntities(2.5, 2.5, 2.5)){
					if(e instanceof Player){
						Player pl = (Player) e;
						damage += 3;
						Server.getInstance().damagePure(pl, 3.0, p);
					}
				}
				if(damage > 0){
	...
	public void Rejuvenate(EntityDamageEvent event){
		...
		if(Kits.getClass((Player) event.getEntity()) == Kits.SQUID){
	...
	public void InnerInk(PlayerItemConsumeEvent event){
		...
		if(event.getItem().getType() == Material.POTION){
			for(Entity e : event.getPlayer().getNearbyEntities(2.5, 2.5, 2.5)){
				if(e instanceof Player){
					Player p = (Player) e;
					Server.getInstance().addNoReductionEffect(p, new PotionEffect(PotionEffectType.BLINDNESS, 60, 2));
				}
			}
		}
		}
	...
	public void EnergyOnBowHit(EntityDamageByEntityEvent event){
		...
		if(event.getEntity().getType() == EntityType.PLAYER && event.getDamager().getType() == EntityType.ARROW){
			...
			if(Kits.getClass(((Player)arrow.getShooter())) == Kits.SQUID){
